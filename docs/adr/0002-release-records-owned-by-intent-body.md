...
Existing unmarked `schema: 1` intent bodies remain supported without rewriting them. Bodies containing the legacy schema and release-intent fields, or releases carrying `verified-release.tgz`, require strict legacy validation; malformed candidates are errors, not foreign releases. Skip only clearly foreign releases during discovery. If a foreign tag or release occupies the exact requested record tag, fail without modifying it.
